...
namespace StockSimulator.Indicators
{
	...
	public class Stochastics : Indicator
	{
		...
		private List<double> _fastK { get; set; }

		private int _periodD = 7;
		private int _periodK = 14;
		private int _smooth = 3;

		public Stochastics(TickerData tickerData, RunnableFactory factory)
			: base(tickerData, factory)
		{
			...
			_fastK = Enumerable.Repeat(0d, Data.NumBars).ToList();
		}
		...
		protected override void OnBarUpdate(int currentBar)
		{
			...
			{
				_fastK[currentBar] = currentBar == 0 ? 50 : _fastK[currentBar - 1];
			}
			else
			{
				_fastK[currentBar] = Math.Min(100, Math.Max(0, 100 * _nom[currentBar] / _den[currentBar]));
			}

			K[currentBar] = UtilityMethods.Sma(_fastK, currentBar, _smooth);
